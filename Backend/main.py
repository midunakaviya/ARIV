...
#     # ─── NEW: Save qaPairs to chatbot_knowledge table ───────────────────────
#     qa_pairs = config.get("qaPairs", [])
#     if isinstance(qa_pairs, list) and qa_pairs:
#         for pair in qa_pairs:
#             question = pair.get("question", "").strip()
#             answer   = pair.get("answer",   "").strip()
#             if question and answer:  # only save non-empty pairs
#                 execute(
#                     """
...
def create_chatbot(data: dict, current_user: dict = Depends(company_required)):
    ...
    public_url = None
    if is_public:
        public_url = f"http://localhost:5173/chat/{bot_id}"  # or use env variable later
        execute(
...
def update_chatbot(chatbot_id: int, data: dict, current_user: dict = Depends(company_required)):
    ...
    public_url = f"http://localhost:5173/chat/{chatbot_id}" if is_public else None
